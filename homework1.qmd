...
# First, create a new dataframe that summarizes the cancellations by month, carrier, and airport origin
canc_summary <- cancellations %>% 
  mutate(origin_group = ifelse(origin == "EWR", "EWR", "JFK")) %>%
  group_by(month, carrier, origin_group) %>% 
  summarise(num_canc = n()) %>% 
  ungroup()

# Then, create a plot with subplots for each origin airport
ggplot(canc_summary, aes(x = month, y = num_canc, group = carrier)) + 
  geom_col() + 
  facet_wrap(origin_group ~ carrier, scales = "free_y") + 
  labs(title = "Cancellations by Month, Carrier, and Origin Airport", x = "Month", y = "Number of Cancellations")
